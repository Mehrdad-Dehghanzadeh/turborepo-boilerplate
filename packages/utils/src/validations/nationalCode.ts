 ...
 * ***********************************/
export function isRealNationalCode(value: string): boolean {
  if (
    !/^\d{10}$/.test(value) ||
    value === '0000000000' ||
    value === '1111111111' ||
 ...
 * ***********************************/
export function isLegalNationalCode(value: string): boolean {
  ...
  let s = 0
  for (let i = 0; i < 10; i++)
    s += (d + parseInt(value.substr(i, 1), 10)) * (z[i % 5] ?? 0)
  s = s % 11
  if (s === 10) s = 0
  return c === s
}
 ...
 * ***********************************/
export function isNationalCode(value: string): boolean {
