      ...
      {payments?.length ? (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
          <Box>
            ...
            <Typography variant="subtitle1">پرداخت‌های مربوط به سفارش</Typography>
          </Box>
            ...
            >
              <KFieldset title="اطلاعات تسهیلات">
                <Grid container>
                  ...
                  <Grid item xs={12} md={6}>
                    <KInfo title="مدت بازپرداخت" value={payment?.lease?.interest} />
                  </Grid>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'end' }}>
                {isAdmin && (
                  <KButton
                    ...
                    color="success"
                    onClick={() => {
                      settlementDialogRef?.current?.showDialog?.(payment)
                    }}
                    disabled={payment.status !== SettlementStates.waitingToSettle}
                  >
                    تسویه
                  </KButton>
                )}
